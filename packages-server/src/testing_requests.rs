use crate::config::{Config, ChannelCfg};
...
use packages::requests::{
	PackageInfoReq,
	SetPackageInfoReq,
	GetFileReq, GetFilePartReq, GetFileBuilder,
	SetFileReq,
	AuthenticateWriter1Req, AuthenticateWriter2Req,
	ChangeWhitelistReq,
	DeviceId
};
use packages::packages::{Package, Channel, BoardArch, TargetArch};
...
fn init() -> TestingServer {
	...
	config.debug = Some(ChannelCfg { sign_key: sign_key.0.public().clone() });
...
async fn auth_as_writer(server: &TestingServer) {
	let resp = server.request(
		AuthenticateWriter1Req { channel: Channel::Debug }
	).await.unwrap();

	let signature = server.get_data::<SignKey>().0
		.sign(resp.challenge);

	let _ = server.request(
		AuthenticateWriter2Req { signature }
	).await.unwrap();
}
...
async fn add_test_package_with_ctn(
	...
	let _ = server.request(
		SetFileReq::from_bytes(signature.clone(), ctn)
	).await.unwrap();
...
async fn test_get_file() {
	...
	let res = server.request(GetFileReq::new(package.version)).await.unwrap();
...
async fn test_get_file_part() {
	...
	// empty request
	let resp = server.request(
		GetFilePartReq::new(package.version.clone(), 0, 0)
	).await.unwrap();
	assert_eq!(resp.file_part().len(), 0);

	// to long request
	let resp = server.request(
		GetFilePartReq::new(package.version.clone(), 0, 1000)
	).await.unwrap();
	assert_eq!(resp.total_file_len(), resp.file_part().len() as u64);

	// out of bound request
	let resp = server.request(
		GetFilePartReq::new(package.version.clone(), 1000, 1000)
	).await;
	...
	for i in 0..=100 {
		let resp = server.request(
			GetFilePartReq::new(package.version.clone(), i * 10, 10)
		).await.unwrap();
...
async fn test_set_package() {
	...
	let _ = server.request(SetPackageInfoReq {
		package,
		whitelist: Default::default(),
		auto_whitelist_limit: 0
	}).await.unwrap();
...
async fn test_whitelist() {
	...
	let device_ids = (0..=100)
		.map(|_| DeviceId::new())
		.collect::<Vec<_>>();

	let package_1 = add_test_package_with_ctn(
		&server,
		"v1.0",
		TEST_FILE_CONTENT
	).await;
	...
	for id in &device_ids {
		let resp = server.request(test_pack_req(id)).await
	...
	let _ = server.request(ChangeWhitelistReq {
		arch: TargetArch::Amd64,
		name: "test".into(),
		version: package_2.version.clone(),
		whitelist: device_ids.iter().rev().take(4)
			.map(Clone::clone)
			.collect(),
		add: true,
		auto_whitelist_limit: 0

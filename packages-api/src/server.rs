...
use stream_api::message::{FromMessage, IntoMessage};
use stream_api::request::{Request, RequestHandler};
use stream_api::server::BuiltServer;
pub use stream_api::server::{Config, EncryptedBytes, Session};
...
type StreamServer<L> =
	stream_api::server::Server<Action, EncryptedBytes, L, Keypair>;
...
impl<L> Server<L> {
	...
	where
		H: RequestHandler<EncryptedBytes, Action = Action>
			+ Send
			+ Sync
			+ 'static,
	{
...
impl Server<TcpListener> {
		...
		A: ToSocketAddrs,
	{
		...
		Ok(Self {
			inner: StreamServer::new_encrypted(
				listener,
				Config {
					timeout: TIMEOUT,
...
impl Server<PanicListener> {
	...
	pub fn build(self) -> TestingServer {
		TestingServer {
			...
			session: Arc::new(Session::new(SocketAddr::V4(
...
impl TestingServer {
	...
	pub fn reset_session(&mut self) {
		self.session = Arc::new(Session::new(SocketAddr::V4(
			"127.0.0.1:8080".parse().unwrap(),
		)));
	}
